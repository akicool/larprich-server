...
Живут во вкладке Environment сервиса в Dokploy, не в файлах на сервере.
Обязательная там ровно одна — `TURN_SHARED_SECRET`; у всего остального в
`docker-compose.prod.yml` есть рабочие значения по умолчанию, а `STUN_URLS` и
`TURN_URLS` вообще выводятся из `DOMAIN_NAME`, чтобы не разъезжались с тем, что
реально отдаёт coturn. Список необязательных переопределений — в
[.env.production.example](.env.production.example).

Секрет задаётся в одном месте: оттуда его берёт и приложение (для выдачи
временных TURN-креденшелов), и coturn (аргументом `--static-auth-secret`). В
репозиторий он не попадает. Если его не задать, деплой падает с внятным
сообщением вместо того, чтобы подняться с пустой конфигурацией.
